...
How it works
PDF is uploaded via a multipart POST request
Apache PDFBox extracts raw text from the PDF server-side
Extracted text (capped at 3000 characters) is sent to Groq's LLM API with a system prompt that forces structured JSON output
The response is parsed into typed fields (documentType, keyTopics, summary) and persisted via Spring Data JPA/Hibernate to MySQL
Full CRUD is exposed over the stored records
API Endpoints
...
Running Locally
Clone the repo, open in IntelliJ (Java 21 required)
Create a MySQL database: document_processing_db
Set environment variables: DB_USERNAME, DB_PASSWORD, GROQ_API_KEY (free key at console.groq.com)
Run DocumentProcessingApplication.java
